...
30 дней (кроме сведений, которые обязан хранить по закону). Плюс App Store
guideline 5.1.1(v) требует настоящее удаление аккаунта, ревью может
завернуть за деактивацию. Предложение: `Storage.DeleteAccount` (или
