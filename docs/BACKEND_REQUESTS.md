...
present». По уточнению Григория ответы бэка приходят моментально — т.е. (а)
было клиентским (стоп-условие поллинга смотрело только на `status` чека,
RPC-сбои молча крутили спиннер; пофикшено на клиенте 2026-07-15). Бэку
остаются два пункта про корректность (не про скорость), разбор по ветке
`ucs` `wswalker`:
...
message.** ApiCloud кладёт статус ошибки в body (`{status: 400, message:
"gosNumber% forbidden symbols present"}`) при HTTP 200.
`ApiCloudResponseNormalizer` пробрасывает `message` как есть, а
`resolveStatus()` смотрит только на HTTP-статус и `found` → чек получает
`not_found`/`success` вместо `error`, и сырой англоязычный текст доезжает до
пользователя. Нужно: если в body есть `status >= 400` / признак ошибки —
ставить `LegalReviewCheckStatus::Error` + машиночитаемый `errorCode`
(например `invalid_gos_number`), а не «не найдено». Клиент с 2026-07-15
переводит известные тексты на русский, но это хрупко — формат ApiCloud может
поменяться.

~~Баг 3 — «вечные pending»~~ **СНЯТО 2026-07-15** после уточнения Григория
(«ответ приходит моментально»): бесконечная загрузка была клиентской, запрос
на reaper не актуален. Теоретическое окно остаётся (упавший NATS-джоб из
`publishBatchRunJobs()` оставил бы чек `pending` навсегда — клиент такой батч
теперь честно таймаутит), вернуть запрос только если в
`legal_review_check` реально найдутся застрявшие pending.
