...
class _MyRequestDetailScreenState extends State<MyRequestDetailScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final title = _data['title'] ?? 'Заявка';
  ...
  List<Widget> _buildByCar() {
    ...
    return cars.map((raw) {
      ...
      final generation = car['generation'] ?? '';
      return Container(
  ...
  Widget _buildOffersSection(
    ...
    final canceled = _canceledOffers();
    final showOthers = status == kStatusCreated;
    return Column(
      key: _offersKey,
      crossAxisAlignment: CrossAxisAlignment.stretch,
      children: [
          ...
          if (showOthers)
            ...offers.map(
              (offer) => _OfferCard(
                ...
                onProfile: () =>
                    _openProfile(offer, onChoose: () => _selectOffer(offer)),
                onChoose: () => _selectOffer(offer),
              ),
            ),
        ],
      ],
