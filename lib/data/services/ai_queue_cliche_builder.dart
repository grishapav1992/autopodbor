...
class AiQueueClicheBuilder {
  ...
  /// Cliché распознавания фото СТС/ПТС (vision: фото уходит через
  /// `fileUrls`). Контракт вывода — строгий JSON, поля зеркалят
  /// `parseDocScanAiResult` (spark_joy_doc_scan_ai.dart): парсер строже
  /// промпта, всё сомнительное отбрасывает. Ключевой гард — «только текст
  /// с фото»: модель НЕ должна достраивать параметры по знанию модели авто
  /// (для этого есть отдельный VIN-конвейер buildVinParamsCliche).
  static String buildDocScanCliche() {
    return 'Ты — система распознавания российских документов на автомобиль: '
        'СТС (свидетельство о регистрации), ПТС (паспорт ТС) или выписка '
        'ЭПТС. На приложенном фото — такой документ. Прочитай ТОЛЬКО то, '
        ...
        '  "engineVolume": рабочий объём двигателя в литрах с одним знаком '
        'после точки (1598 см³ → 1.6) или "",\n'
        '  "engineType": ОДНО из ["Бензин","Дизель","Гибрид","Электро",'
        '"Газ/Бензин"] или ""\n'
        '}\n'
        'Enum-поля пиши ДОСЛОВНО как в списках выше. Поле не читается, '
        'обрезано или в документе отсутствует → "". НЕ угадывай и НЕ '
        'дополняй по общим знаниям об этой модели авто — только текст с '
        'фото. Если на фото не документ ТС — верни все поля пустыми и '
